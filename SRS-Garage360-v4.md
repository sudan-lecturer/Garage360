...
| 4.0.0 | Redesigned job card lifecycle: intake inspection + customer signature, service bay management, mid-service change requests, QA linked to intake, account manager role, job activity timeline, notification channel per job |
...
9. Module Specifications (detailed)
   ...
   - 9.3 Job Card Lifecycle *(fully redesigned)*
     - 9.3.1 Intake Inspection & Customer Signature *(new)*
     - 9.3.2 Service Bay Management *(new)*
     - 9.3.3 Estimation & Customer Approval
     - 9.3.4 Mid-Service Change Requests *(new)*
     - 9.3.5 Post-Service QA (linked to intake)
     - 9.3.6 Job Card Activity Timeline *(new)*
...
| Notifications | `lettre` (email) + `reqwest` (SMS gateway) | Staff and customer notifications |
| PDF generation | `printpdf` + `lopdf` | Intake report PDF + invoice PDF |
...
| `jobs.intake_inspection` | `true` | Vehicle intake checklist before job card |
| `jobs.intake_signature` | `true` | Customer digital signature on intake |
| `jobs.bay_management` | `true` | Service bay tracking and board view |
| `jobs.approval_workflow` | `true` | Quote approval step |
| `jobs.mid_service_approval` | `true` | Customer approval on mid-service additions |
| `jobs.dvi_required` | `false` | Block billing until post-service QA complete |
| `jobs.notification_sms` | `false` | SMS notifications to customers |
| `jobs.notification_email` | `true` | Email notifications to customers |
...
| `MANAGER` | Tenant | Jobs full lifecycle, inventory, purchasing approval, bay management, staff scheduling |
| `ACCOUNT_MANAGER` | Tenant | Create/edit estimates (QUOTE stage), generate invoices, customer communication, read-only job view |
| `MECHANIC` | Tenant | Assigned jobs (IN_SERVICE), intake checklist, DVI, asset daily inspection, defect reporting, change request submission |
| `CASHIER` | Tenant | Invoicing, payment receipt, read-only job and customer view |
...
### 9.3 Job Card Lifecycle *(fully redesigned in v4.0)*

---

#### 9.3.0 Revised Status Flow

```
INTAKE → AUDIT → QUOTE → APPROVAL → IN_SERVICE → QA → BILLING → COMPLETED
                                               ↕                ↘ CANCELLED
                                        (change requests
                                         loop within IN_SERVICE)
```

- `INTAKE` is the new first status. Job card created as soon as customer arrives.
- `INTAKE → AUDIT` requires: intake checklist complete + customer signature captured.
- `AUDIT → QUOTE` requires: complaint recorded, mechanic assigned, bay assigned.
- `QUOTE → APPROVAL` requires: at least one line item on the estimate.
- `APPROVAL → IN_SERVICE` requires: customer approval recorded.
- `IN_SERVICE → QA` requires: mechanic marks work complete.
- `QA → BILLING` requires: QA pass (all checklist items resolved).
- `QA fail` returns job to `IN_SERVICE` with defect notes.
- `APPROVAL` step skipped if `jobs.approval_workflow = false`.
- `QA` step skipped if `jobs.dvi_required = false` (not recommended).
- `CANCELLED` reachable from any status; requires reason + Manager+ role.

All transitions enforced in Rust service layer. Invalid transitions return `422 Unprocessable Entity`.
...
##### Business Rules
- When a customer arrives, the service manager opens a new job card at status `INTAKE`.
- The intake inspection is a structured checklist completed by the service manager alongside the customer at the vehicle.
- Intake checklist items are configurable per tenant but every tenant starts with a standard default set.
- Vehicle photos are optional but strongly encouraged — they become the "before" reference for post-service QA and protect the workshop in damage disputes.
- Customer must sign digitally before the job proceeds. The signature legally confirms the vehicle's recorded condition at drop-off.
- The signed intake document is printable as a PDF and can be automatically emailed to the customer.

##### Default Intake Checklist Items

| Item | Response Type | Mandatory |
|---|---|---|
| Keys handed over | Pass/Fail + count | Yes |
| Front-left tyre pressure | Numeric (PSI/Bar) | Yes |
| Front-right tyre pressure | Numeric (PSI/Bar) | Yes |
| Rear-left tyre pressure | Numeric (PSI/Bar) | Yes |
| Rear-right tyre pressure | Numeric (PSI/Bar) | Yes |
| Engine condition (visual) | Pass/Fail + Notes | Yes |
| Odometer reading | Numeric (km) | Yes |
| Headlights functional | Pass/Fail | Yes |
| Tail lights functional | Pass/Fail | Yes |
| Interior — belongings on seat / in vehicle | Text (list items noted) | Yes |
| Accident / damage history (customer declared) | Pass/Fail + Notes | Yes |
| Fuel level | Enum (Empty/Quarter/Half/ThreeQuarter/Full) | No |
| Dashboard warning lights | Pass/Fail + Notes | No |
| Spare tyre present | Pass/Fail | No |

Tenant Admin can add, remove (non-mandatory only), or reorder items via Settings → Intake Template.

##### Vehicle Photos at Intake
- Up to 10 photos per job at intake (device camera PWA API or file upload).
- Photo prompts: Front, Rear, Driver side, Passenger side, Any existing damage.
- Each photo optionally annotated with a short note.
- Stored in MinIO/S3, URL + metadata in `intake_photos`.
- Displayed on job card detail, QA screen (as "before" reference), and printed PDF.

##### Customer Digital Signature
- After checklist completion, service manager presents summary to customer on device.
- Customer signs on screen (touch/stylus/mouse) using a signature pad component.
- Signature captured as SVG path, stored as PNG in object storage.
- Signed intake PDF generated: checklist results + vehicle photo thumbnails + signature.
- PDF auto-emailed to customer if email is on file (flag: `jobs.notification_email`).
- PDF printable at any time from the job card.

##### Tables
```
intake_checklist_templates    — id, location_id, items (JSON array), updated_at
intake_checklists             — id, job_card_id, template_id, completed_by, completed_at
intake_checklist_responses    — id, checklist_id, item_key, response_type, value, notes
intake_photos                 — id, job_card_id, url, caption, sort_order, taken_at, taken_by
customer_signatures           — id, job_card_id, png_url, svg_path, signed_by_name,
                                signed_at, ip_address, device_info
```

##### API Routes
```
GET    /settings/intake-template          Get intake template for location (Admin+)
PUT    /settings/intake-template          Update intake template (Admin+)
POST   /jobs/:id/intake/checklist         Save intake checklist responses
POST   /jobs/:id/intake/photos            Upload intake photo(s)
DELETE /jobs/:id/intake/photos/:photoId   Remove intake photo
POST   /jobs/:id/intake/signature         Capture customer signature
GET    /jobs/:id/intake/report            Download/email signed intake PDF
POST   /jobs/:id/intake/complete          Complete intake (triggers INTAKE → AUDIT)
```
...
##### Business Rules
- A service bay is a physical workspace within a location where vehicle service is performed.
- Bay count is configured per location by Admin/Owner — no system limit.
- A bay can only have one active job at a time (enforced server-side).
- Bay status is computed from active job cards — never stored.
- Bay board gives the service manager a live operational view.

##### Bay Status (Computed)
| Status | Color | Condition |
|---|---|---|
| AVAILABLE | `#22C55E` | No active job assigned |
| RESERVED | `#F59E0B` | Job assigned but not yet IN_SERVICE |
| OCCUPIED | `#EF4444` | Job currently IN_SERVICE |
| OUT_OF_SERVICE | `#6B7280` | Manually marked unavailable |

##### Estimated Time to Free
- Manager sets `estimated_completion_at` when job enters IN_SERVICE.
- Bay board shows "Free at 14:30" or "Overdue by 45 min".
- Mechanic can update their own job's estimated completion time.
- Bay board auto-refreshes (30-second polling or SSE).

##### Bay Board View
- Dedicated `/bays` route + dashboard widget.
- Card grid: bay code, status badge, job no, customer name, license plate, mechanic name, est. free time.
- Manager can reassign job to different bay from this screen.
- Mobile: card stack, same data, full-touch layout.

##### Tables
```
service_bays   — id, location_id, bay_code, name, notes, is_active, created_at
```

##### Job Card Fields Added
```
bay_id                   UUID FK → service_bays  (nullable)
estimated_completion_at  TIMESTAMPTZ              (set on IN_SERVICE, updatable)
```

##### API Routes
```
GET    /settings/bays              List bays for location
POST   /settings/bays              Create bay (Admin+)
PUT    /settings/bays/:id          Update bay (Admin+)
PUT    /settings/bays/:id/status   Mark OUT_OF_SERVICE / restore (Admin+)
DELETE /settings/bays/:id          Soft-delete bay (Admin+)
GET    /bays/board                 Live bay board data for location (Manager+)
PUT    /jobs/:id/bay               Assign / reassign bay
PUT    /jobs/:id/estimated-completion  Update estimated completion
```

---

#### 9.3.3 Estimation (QUOTE) & Customer Approval

After mechanic records diagnosis at AUDIT, job moves to QUOTE. The Account Manager builds the estimate: selects parts from inventory, adds labour and sublet items, and submits for customer approval.

When job moves to `APPROVAL`, the system notifies the customer via their preferred channel (set at intake on the job card):
- **Email** — itemised estimate as HTML email
- **SMS** — short message with total + portal link (if `module.customer_portal` enabled)
- **In-Person / Phone** — staff records verbal approval manually

Every approval event is recorded in `job_card_approvals`:
```
id, job_card_id, approval_type (ESTIMATE | CHANGE_REQUEST),
notification_channel (EMAIL | SMS | IN_PERSON | PHONE),
notified_at, notified_by, approved_at, approved_by_name, approval_notes, recorded_by
```
...
##### Business Rules
- During `IN_SERVICE`, mechanic may find additional work needed beyond the approved estimate.
- All additions require customer approval before the mechanic proceeds.
- Job card stays at `IN_SERVICE` — no status change during change requests.
- Multiple change requests can be open simultaneously.
- Approved items automatically added to job card line items and stock reserved.

##### Change Request Lifecycle
```
PENDING_APPROVAL → APPROVED → items added to job card
                ↘ DECLINED  → noted on activity timeline, mechanic notified
```

##### Tables
```
job_change_requests       — id, job_card_id, requested_by, description, status,
                            notification_channel, notified_at, resolved_at,
                            resolved_by, resolution_notes
job_change_request_items  — id, change_request_id, inventory_item_id (nullable),
                            description, quantity, unit_price, type (PART|LABOUR|SUBLET)
```

##### API Routes
```
GET    /jobs/:id/change-requests           List all change requests
POST   /jobs/:id/change-requests           Create change request (Mechanic+)
PUT    /jobs/:id/change-requests/:crId     Approve or decline (Manager / Account Manager)
POST   /jobs/:id/change-requests/:crId/notify  Resend customer notification
```
...
##### Business Rules
- Job moves to `QA` when mechanic marks work complete.
- QA performed by a different person than the performing mechanic (configurable enforcement).
- QA screen shows intake checklist results and intake photos as "before" reference side-by-side with QA items.
- QA must cover: all original complaint items, all approved change request items, key intake re-checks (tyre pressures, lights, cleanliness, keys).
- **PASS** → job advances to BILLING.
- **FAIL** → job returns to IN_SERVICE. `qa_failure_notes` record created and assigned to mechanic. `qa_cycles` counter incremented.
- QA cycles tracked per job for quality reporting.

##### API Routes
```
GET    /jobs/:id/qa/context    Intake data + complaint items + change requests for QA screen
POST   /jobs/:id/qa/submit     Submit QA result (PASS or FAIL with per-item notes)
GET    /jobs/:id/qa/history    All QA attempts for this job
```
...
Every significant event on a job card is recorded as an immutable, timestamped, user-stamped activity entry. Visible to all roles with read access. Append-only — never updated or deleted.

##### Activity Types
`JOB_CREATED`, `INTAKE_COMPLETED`, `SIGNATURE_CAPTURED`, `STATUS_CHANGED`, `MECHANIC_ASSIGNED`, `BAY_ASSIGNED`, `ITEM_ADDED`, `ITEM_REMOVED`, `ESTIMATE_SENT`, `ESTIMATE_APPROVED`, `CHANGE_REQUEST_RAISED`, `CHANGE_REQUEST_APPROVED`, `CHANGE_REQUEST_DECLINED`, `CUSTOMER_NOTIFIED`, `QA_SUBMITTED`, `QA_FAILED`, `INVOICE_GENERATED`, `PAYMENT_RECEIVED`, `NOTE_ADDED`

##### Table
```
job_card_activities — id, job_card_id, activity_type (enum), performed_by,
                      performed_by_name (snapshot), description, metadata (JSON),
                      created_at
...
##### API Routes
```
GET    /jobs/:id/activities         Full chronological timeline
POST   /jobs/:id/activities/note    Add manual staff note
```

---

#### 9.3.7 Complete Job Card Fields (v4.0)

All v3.0 fields retained. Additions:

| Field | Type | Notes |
|---|---|---|
| `status` | enum | INTAKE added as first status |
| `bay_id` | UUID FK → service_bays | Assigned at AUDIT stage |
| `estimated_completion_at` | TIMESTAMPTZ | Set on IN_SERVICE entry |
| `preferred_contact_channel` | enum | EMAIL, SMS, PHONE — set at intake |
| `intake_checklist_id` | UUID FK | Links completed intake_checklists record |
| `signature_id` | UUID FK | Links customer_signatures record |
| `qa_cycles` | integer | Count of QA pass/fail cycles on this job |
| `account_manager_id` | UUID FK | ACCOUNT_MANAGER user owning the estimate |
| `customer_type_snapshot` | enum | Individual/Organisation at time of job |
| `license_plate_snapshot` | text | License plate captured at intake |

---

#### 9.3.8 Full Job API Routes
```
GET    /jobs                             List + filter (status, bay, mechanic, location, date, plate)
POST   /jobs                             Create job card (status = INTAKE)
GET    /jobs/:id                         Full detail (intake, timeline, bays, change requests, QA)
PUT    /jobs/:id                         Update job metadata
POST   /jobs/:id/transition              Advance status (validated state machine)
PUT    /jobs/:id/assign-mechanic         Assign / reassign mechanic
PUT    /jobs/:id/assign-bay              Assign / reassign bay
PUT    /jobs/:id/assign-account-manager  Assign account manager
GET    /jobs/:id/activities              Activity timeline
POST   /jobs/:id/activities/note         Add note to timeline
GET    /jobs/:id/intake/report           Download signed intake PDF
GET    /jobs/search                      Search by job no, customer name, license plate
GET    /jobs/export                      Export to Excel (respects active filters)
```
...
**Workshop — Job Cards:** `job_cards`, `job_card_items`, `job_card_activities` *(new)*, `job_card_approvals` *(new)*, `job_change_requests` *(new)*, `job_change_request_items` *(new)*

**Workshop — Intake:** `intake_checklist_templates` *(new)*, `intake_checklists` *(new)*, `intake_checklist_responses` *(new)*, `intake_photos` *(new)*, `customer_signatures` *(new)*

**Workshop — Bays:** `service_bays` *(new)*
...
-- Job card activities (timeline queries)
CREATE INDEX job_activities_job_id_idx ON job_card_activities(job_card_id, created_at);
...
-- Change requests
CREATE INDEX change_requests_job_id_status_idx ON job_change_requests(job_card_id, status);
```
...
│   │   ├── jobs/                — job card state machine + all sub-resources
│   │   │   ├── intake/          — checklist, photos, signature, PDF generation
│   │   │   ├── bays/            — bay management + board view
│   │   │   ├── change_requests/ — mid-service approval loop
│   │   │   ├── activities/      — append-only timeline
│   │   │   └── qa/              — post-service QA linked to intake
│   │   ├── inventory/           — CRUD + stock alerts + adjustments
│   │   ├── purchases/           — PO lifecycle + GRN + QA + stock-in
│   │   ├── billing/             — invoice generation + payment
│   │   ├── dvi/                 — DVI templates + results + photos
...
│   │   └── settings/            — tenant config + users + locations + intake template + bays
│   ├── control/
│   │   ├── tenants/
│   │   └── feature_flags/
│   ├── jobs/                    — Background job runners (import, notifications, PDF gen)
│   ├── notifications/           — Push + email + SMS notification dispatch
│   ├── storage/                 — Object storage abstraction (MinIO/S3)
│   ├── pdf/                     — PDF generation (intake reports, invoices)
...
│   ├── dashboard/
│   │   └── components/
│   │       ├── BayBoardWidget.tsx       — Live bay status overview
│   │       └── StockAlertWidget.tsx
│   ├── customers/
│   ├── vehicles/
│   ├── jobs/
│   │   ├── components/
│   │   │   ├── IntakeChecklistForm.tsx  — Configurable intake checklist
│   │   │   ├── IntakePhotoCapture.tsx   — Camera/upload for intake photos
│   │   │   ├── SignaturePad.tsx         — Touch/mouse signature capture
│   │   │   ├── BaySelector.tsx          — Available bay picker
│   │   │   ├── JobStatusStepper.tsx     — Visual status flow stepper
│   │   │   ├── ActivityTimeline.tsx     — Chronological job events
│   │   │   ├── ChangeRequestModal.tsx   — Mid-service addition request
│   │   │   ├── QAScreen.tsx             — QA form with intake reference panel
│   │   │   └── EstimateBuilder.tsx      — Parts + labour estimate builder
│   │   └── routes/
│   │       ├── JobList.tsx
│   │       ├── JobDetail.tsx
│   │       ├── JobIntake.tsx            — Intake flow (multi-step)
│   │       └── JobQA.tsx
│   ├── bays/
│   │   └── BayBoard.tsx                 — Full bay board view (/bays route)
│   ├── inventory/
│   ├── purchases/
│   ├── billing/
│   ├── dvi/
...
│   ├── import-export/
│   └── settings/
│       ├── IntakeTemplateEditor.tsx     — Configure intake checklist items
│       ├── BayManagement.tsx            — CRUD for service bays per location
...
│       ├── SignaturePad.tsx             — Reusable signature component
│       └── CameraCapture.tsx           — Reusable camera/upload component
```
...
|---|---|---|---|
| 1 | `schema-architect` | Needs full update (v4 tables: intake, bays, change requests, activities, signatures) | P0 |
| 2 | `rust-module-builder` | Build next | P0 |
| 3 | `react-module-builder` | After rust skill | P1 |
| 4 | `docker-composer` | Full docker-compose + Dockerfiles + nginx | P1 |
| 5 | `pwa-configurator` | Vite PWA, SW strategy, camera API, signature pad | P2 |
| 6 | `rbac-enforcer` | Axum middleware + frontend flag/role gating + ACCOUNT_MANAGER role | P1 |
| 7 | `excel-io-builder` | calamine + rust_xlsxwriter + SheetJS patterns | P2 |
| 8 | `test-writer` | Rust integration tests + RTL tests | P2 |
| 9 | `notification-builder` | Push + email + SMS dispatch patterns | P2 |
| 10 | `search-builder` | PostgreSQL FTS with tsvector, multi-field search API | P2 |
| 11 | `pdf-builder` | Intake report PDF + invoice PDF generation in Rust | P2 |
| 12 | `state-machine-builder` | Job card transition validator in Rust service layer | P1 |
